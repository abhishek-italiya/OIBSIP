...
- **Outlier Analysis & Treatment:** Implement Interquartile Range (**IQR**) outlier detection and Winsorization (capping).
...
| `Purchase_Amount` | `float64` | `float64` | Transaction order amount in USD ($) |
...
| `Purchase_Amount_Capped` | N/A | `float64` | IQR-winsorized purchase amount for outlier stability |
...
| **Purchase Amount Outliers** | Negative values (`-$50.00`) and high spikes (`$15,000.00`) | Filtered negatives and Winsorized at IQR Upper Bound ($670.36) |
...
- **Outlier Decision:** Capped extreme spikes ($15,000.00) at $670.36 (**Winsorization**) to protect variance while retaining customer transaction rows.
...
| **Purchase Amount Outliers** | Extreme spikes ($15,000.00) | **Winsorized at Upper IQR Bound ($670.36)** |
...
3. **`outliers_boxplots_before_after.png`:** Side-by-side boxplots illustrating raw outlier spikes vs winsorized purchase amounts.
...
│   └── reports/                        # Audit and summary report CSVs
...
- **Data Preservation via Winsorization:** Deleting outliers reduces sample size, whereas IQR capping preserves observation counts while stabilizing metric variance.
